    ...
    return (
      <div className="flex flex-col items-center justify-center w-full min-h-screen bg-gray-100">
        <Head>
          ...
          <meta
            name="description"
            content="This Singlelink is under construction!"
          />
        ...
        <div className="flex flex-col items-center justify-center w-full max-w-md p-8 rounded-xl text-center">
          ...
          <p className=" mb-6 text-gray-600">
            This{" "}
            <a
              href="https://singlelink.co"
              target="_blank"
              className="font-semibold text-indigo-600 hover:underline hover:text-indigo-700"
              rel="noreferrer"
            >
              Singlelink
            </a>{" "}
            page is currently under construction. Be sure to check back soon!
          </p>
          <Link passHref href="/login">
            <button className="px-8 py-4 w-full bg-indigo-600 w-full text-white font-semibold rounded-xl hover:bg-indigo-700">
              Log in to my Singlelink dashboard
            </button>
          </Link>
        </div>
        <div className="text-gray-600 mt-12 text-xs">
          Copyright ©{new Date().getFullYear()} Neutron Creative Inc. All rights
          reserved.
        </div>
      </div>
    );
    ...
    <>
      ...
      <div className="flex flex-col items-center justify-center w-full max-w-md mx-auto py-16 px-4">
        {[...links]
          ...
          .map((link) => {
            if (link?.type === "avatar")
              return (
                <div
                  className="text-center flex flex-col items-center justify-center rounded-full mb-4"
                  key={link?.id}
                >
                  <img
                    alt="User"
                    src={link.content ?? "#"}
                    className="avatar-pic flex items-center justify-center w-24 h-24 rounded-full"
                  />
                </div>
              );
            if (link?.type === "text")
              return (
                <div
                  className="text-center flex flex-col items-center justify-center mb-4"
                  key={link?.id}
                >
            ...
            if (link?.type === "vanilla" && link?.label === "GitHub")
              return (
                <a
                  href={link.content ?? "#"}
                  className="shadow hover:scale-105 duration-300 ease-in-out flex flex-col w-full text-center px-3 py-5 mb-3 bg-white text-black text-lg font-medium rounded-lg link-name"
            ...
            if (link?.type === "vanilla" && link?.label === "Twitter")
              return (
                <a
                  href={link.content ?? "#"}
                  className="shadow hover:scale-105 duration-300 ease-in-out flex flex-col w-full text-center px-3 py-5 mb-3 bg-white text-black text-lg font-medium rounded-lg link-name"
            ...
            if (link?.type === "vanilla" && link?.label === "Facebook")
              return (
                <a
                  href={link.content ?? "#"}
                  className="shadow hover:scale-105 duration-300 ease-in-out flex flex-col w-full text-center px-3 py-5 mb-3 bg-white text-black text-lg font-medium rounded-lg link-name"
            ...
            if (link?.type === "vanilla" && link?.label === "Twitch")
              return (
                <a
                  href={link.content ?? "#"}
                  className="shadow hover:scale-105 duration-300 ease-in-out flex flex-col w-full text-center px-3 py-5 mb-3 bg-white text-black text-lg font-medium rounded-lg link-name"
            ...
            if (link?.type === "vanilla")
              return (
                <a
                  href={link.content ?? "#"}
                  className="shadow hover:scale-105 duration-300 ease-in-out flex flex-col w-full text-center px-3 py-5 mb-3 bg-white text-black text-lg font-medium rounded-lg"
                  key={link?.id}
                >
                  {link?.label}
                </a>
        ...
        {process.env.BRANDING && (
          <div className="text-gray-600 mt-auto mt-10 text-center">
            Build your free micro-site in seconds with{" "}
            <a
              href="https://singlelink.co"
  ...
  if (req.url.substr(req.url.length - 9) != "demo=true") {
    ...
    await fetch(`${origin}/api/graphql`, {
      headers: {
        ...
        authorization:
          "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE2NDQ5MDA1ODB9.XqQEdkcgek6it_BIwfX8OAUEUE40ylW2KTdCE71jcEw",
        "content-type": "application/json",
        "sec-ch-ua":
